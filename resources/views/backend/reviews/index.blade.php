...
{{-- Bulk toolbar + table in one form --}}
<div class="card">
    <form id="bulkForm" method="POST" action="{{ route('reviews.bulk-destroy') }}">
        @csrf
        @method('DELETE')
                ...
                @forelse ($reviews as $review)
                    <tr>
                        <td class="align-middle">
                            <div class="form-check m-0">
                                <input class="form-check-input js-bulk" type="checkbox" name="ids[]"
                                       value="{{ $review->id }}">
                            </div>
                        </td>
...
(function () {
    ...
    const $checks        = () => Array.from(document.querySelectorAll('.js-bulk'));
    ...
    function updateState() {
        ...
        const allOn    = checks.length > 0 && checked.length === checks.length;
    ...
    document.addEventListener('change', function (e) {
        if (e.target && e.target.classList.contains('js-bulk')) {
            updateState();
        }
    });
