...
$(document).ready(function () {
  ...
  $('.menu-item').hover(
    ...
    function () {
      const itemId = $(this).attr('id').split('-')[2];
      $(`#sub-menu-${itemId}`).stop(true, true).slideUp(300);
    }
  );

  $('.sub-menu-item').hover(
    function () {
      const parentId = $(this).closest('.sub-menu').attr('id').split('-')[2];
      const subItemId = $(this).attr('id').split('-')[2];

      $(`#inner-menu-${parentId}-${subItemId}`).stop(true, true).slideDown(300);
    },
    function () {
      const parentId = $(this).closest('.sub-menu').attr('id').split('-')[2]; 
      const subItemId = $(this).attr('id').split('-')[2]; 
      ...
      $(`#inner-menu-${parentId}-${subItemId}`).stop(true, true).slideUp(300);
    }
  );
...
document.addEventListener("DOMContentLoaded", function() {
  const target = document.getElementById('sub-menu-v-pills-menu-tab');
  if (target) {
